...
const App = () => {
  ...
  const [currJokeID, setCurrJokeID] = useState(0)
  const randomizeJokeID = () => {
    setCurrJokeID(Math.floor(Math.random() * jokes.length))
  }
  const increaseJokeLikes = (currJokeID) => {
    setJokes(prevJokes => {
      const newJokes = [...prevJokes]
      newJokes[currJokeID] = {
        ...newJokes[currJokeID],
        likes: newJokes[currJokeID].likes + 1
      }
  ...
  return (
    <div>
      ...
      <p>{jokes[currJokeID].joke}</p>
      <p>Likes: {jokes[currJokeID].likes}</p>
      <button onClick={() => { increaseJokeLikes(currJokeID) }}>Like</button>
      <button onClick={randomizeJokeID}>Next Joke</button>
      <MostLikedJoke />
    </div>
  )
}
